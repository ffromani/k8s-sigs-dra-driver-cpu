...
var _ = ginkgo.Describe("CPU Assignment", ginkgo.Serial, ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.BeforeAll(func(ctx context.Context) {
		...
		rootFxt.Log.Info("checking worker node", "coreCount", len(targetNodeCPUInfo.CPUs), "availableCPUs", availableCPUs.String())
	})
	...
	ginkgo.When("setting resource claims", func() {
		...
		ginkgo.Context("for exclusive CPU allocation", func() {
			...
			ginkgo.It("should allocate exclusive CPUs, remove from the shared pool, release on pod deletion", func(ctx context.Context) {
				...
				sharedPod, err := e2epod.CreateSync(ctx, fxt.K8SClientset, makeTesterPodBestEffort(fxt.Namespace.Name, dracpuTesterImage))
				...
				rootFxt.Log.Info("checking shared allocation", "pod", sharedPod.Namespace+"/"+sharedPod.Name, "cpuAllocated", sharedAlloc.CPUAssigned.String(), "cpuAffinity", sharedAlloc.CPUAffinity.String())
				unaccountedCPUs := availableCPUs.Difference(sharedAlloc.CPUAssigned)
				gomega.Expect(unaccountedCPUs.Size()).To(gomega.BeZero(), "available CPUs not in the shared pool: %v", unaccountedCPUs.String())

				ginkgo.By("creating a guaranteed pod getting exclusive CPUs from the resource claim")
				exclPod, err := e2epod.CreateSync(ctx, fxt.K8SClientset, makeTesterPodWithExclusiveCPUClaim(fxt.Namespace.Name, dracpuTesterImage, exclCPUClaim))
				gomega.Expect(err).ToNot(gomega.HaveOccurred(), "cannot create tester pod: %v", err)
				...
				rootFxt.Log.Info("checking exclusive allocation", "pod", exclPod.Namespace+"/"+exclPod.Name, "cpus", exclusiveAlloc.CPUAssigned.String())
				...
				gomega.Eventually(func() error {
					sharedAlloc := getTesterPodCPUAllocation(fxt.K8SClientset, ctx, sharedPod)
					if !expectedSharedCPUs.Equals(sharedAlloc.CPUAssigned) {
						return fmt.Errorf("shared CPUs mismatch: expected %v got %v", expectedSharedCPUs.String(), sharedAlloc.CPUAssigned.String())
					}
					return nil
