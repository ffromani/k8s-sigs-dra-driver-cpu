...
func PCIeDomainsFromFS(logger logr.Logger, sysfs SysFS) ([]PCIeDomain, error) {
	...
	err := ScanPCIeDevices(logger, sysfs, func(pciDev PCIeDevice) error {
		...
		plogger.V(6).Info("PCIe: candidate bridge found")
		...
		plogger.V(6).Info("PCIe: candidate bridge", "localCPUs", localCPUs.String())
		...
		plogger.V(6).Info("PCIe: candidate bridge", "numaNode", numaNode)
		...
		plogger.V(6).Info("PCIe: candidate bridge mapped to domain", "domain", pcd.String())
