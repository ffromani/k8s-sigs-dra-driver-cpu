...
func populateTopologyInfo(cpuInfo *CPUInfo, logger logr.Logger) error {
	...
	if err == nil {
		...
		if err != nil {
			logger.V(2).Info("could not parse cluster_id", "clusterStr", clusterStr, "cpuID", cpuID, "err", err)
			cpuInfo.ClusterID = -1
		} else {
	...
	for _, file := range files {
		if strings.HasPrefix(file.Name(), "node") {
			...
			if err != nil {
				logger.V(2).Info("could not parse NUMA node ID", "entry", file.Name(), "err", err)
				continue
			}
			...
			if err != nil || len(cpuListLines) == 0 {
				logger.V(2).Info("could not read NUMA affinity mask", "nodeID", nodeID, "err", err)
				continue
			}
			cpuInfo.NumaNodeCPUSet, err = cpuset.Parse(cpuListLines[0])
			if err != nil {
				logger.V(2).Info("could not parse NUMA affinity mask", "nodeID", nodeID, "err", err)
				continue
			}
			break
		}
